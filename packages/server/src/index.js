...
module.exports = function createServer(options) {
  options = Object.assign({}, { http: { port: 9000, root: ['./'], rewrite: {} }, rewrite: {}, instrument: { exclude: '**' }, transform: { exclude: '**' } }, options); //eslint-disable-line
  ...
  Object.keys(options.http.rewrite).forEach(key => app.use(rewrite(key, options.rewrite[key])));
