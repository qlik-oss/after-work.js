...
/**
 * Helper function reference for printing to stdout and also we only need to disable eslint no-console once...
 */
...
function buildFailedString( results ) {
	return "\u2717 FAILED:" + results.filter( result => {
...
function parseResults( results ) {
	...
	results = results.sort( ( a, b ) => {
		return a === "fail" ? -1 : b === "fail" ? 0 : 1;
	} );

	// assume everything went ok before we actually parse the result...
	let processExitCode = EXIT_CODES.ok;
	let hasFailed = results.filter( elem => { return elem.status === "fail"; } ).length > 0;

	for ( let i = 0; i < results.length; i++ ) {
		const result = results[ i ];

		for ( let message in result.text ) {
