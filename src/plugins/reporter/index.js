...
export default function uiReport(runner, options) {
  ...
  if (options.reporterOptions && options.reporterOptions.xunit) {
    const xunitName = `${info.browserName}-report-${info.startTime}.xml`;
    options.reporterOptions.output = path.resolve(artifactsPath, xunitName);
  ...
  runner.on('end', function onEnd() {
    ...
    const reportName = `${info.browserName}-report-${info.startTime}.json`;
    const fileName = path.resolve(artifactsPath, reportName);
