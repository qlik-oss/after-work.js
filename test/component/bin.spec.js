...
describe( "bin", () => {
	...
	beforeEach( () => {
		...
		sandbox.stub( path, "resolve", ( arg1, arg2 ) => {
			return arg2;
		} );
		sandbox.stub( path, "relative", ( arg1, arg2 ) => {
			return arg2;
		} );
		on = sandbox.stub();
	...
	describe( "`aw-test-runner`", () => {
		...
		describe( "win32", () => {
			...
			it( "should run mocha with default args", () => {
				...
				expect( spawn ).to.have.been.calledWith( command + commandExt, envConfig.args.concat( defaultArgs ), spawnOptions );
			} );
