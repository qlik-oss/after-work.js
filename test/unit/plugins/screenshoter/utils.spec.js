...
describe( "Screenshoter Utils", () => {
	...
	describe( "compare", () => {
		...
		it( "should be equal if the tolerance is met", () => {
			...
			jimpDiff.returns( { image: diffImg, percent: percent } );
	...
	describe( "getBoundingClientRect", () => {
		...
		it( "should return rect for valid selector", () => {
			...
			querySelector.returns( Object.create( Element.prototype, { getBoundingClientRect: { value: getBoundingClientRect } } ) );
	...
	describe( "takeImageOf", () => {
		...
		beforeEach( () => {
			browser = {
				executeAsyncScript: sandbox.stub().returns( Promise.resolve( { left: 200, top: 100, width: 200, height: 100 } ) ),
			...
			sandbox.stub( jimp, "read", () => {
				crop = sandbox.stub();
				return Promise.resolve( { crop: crop } );
			} );
		} );

		it( "should resolve if it was possible to take an image with selector", () => {
			return utils.takeImageOf( browser, { selector: "my-selector" } ).then( result => {
		...
		it( "should resolve if it was possible to take an image with default selector", () => {
			return utils.takeImageOf( browser, {} ).then( result => {
				expect( result.rect ).to.deep.equal( { top: 100, left: 200, width: 200, height: 100 } );
				expect( result.browserName ).to.equal( "chrome" );
		...
		it( "should add offset to the rect used for cropping", () => {
			return utils.takeImageOf( browser, { selector: "my-selector", offsetX: -5, offsetY: -5, offsetWidth: 5, offsetHeight: 5 } ).then( result => {
	...
	describe( "matchImageOf", () => {
		let chaiCtx, matchImageOf, fileExists, writeImage, compare, mkdir;
		let baselinePath = "artifacts/baseline/";
		let baseline = baselinePath + "id-windows-nt-chrome.png";
		let regressionPath = "artifacts/regression/";
		let regression = regressionPath + "id-windows-nt-chrome.png";
		let diffPath = "artifacts/diff/";
		let diff = diffPath + "id-windows-nt-chrome.png";
		let img = {};

		beforeEach( () => {
			sandbox.stub( path, "resolve", ( ...args ) => { return args.join( "/" ).replace( "//", "/" ); } );
			fileExists = sandbox.stub( utils, "fileExists" );
			writeImage = sandbox.stub( utils, "writeImage" );
			compare = sandbox.stub( utils, "compare" );
			chaiCtx = { _obj: Promise.resolve( { img: img, browserName: "chrome", artifactsPath: "artifacts", platform: "windows-nt" } ), assert: sinon.stub() };
		...
		it( "should reject if not considered equal to baseline", () => {
			...
			fileExists.returns( Promise.resolve( true ) );
			writeImage.returns( Promise.resolve() );
			compare.returns( Promise.resolve( { isEqual: false, diffImg: diffImg } ) );
			return matchImageOf( "id" ).then( () => {
